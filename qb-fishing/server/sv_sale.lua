...
RegisterNetEvent('qb-fishing:server:SellIllegalFish', function(fish, amount)
    ...
    local item = Player.Functions.GetItemByName(fish)
    if not item then return end
    if not illegalPrice[fish] then return end
    if item.amount >= amount then
        if exports['qb-inventory']:RemoveItem(Player.PlayerData.source, item.name, amount, false) then
            -- Item Box
            TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items[item.name], 'remove', amount)
            ...
            Player.Functions.AddItem('bands', amount * illegalPrice[item.name], false, info)

            -- Log
            TriggerEvent('qb-log:server:CreateLog', 'fishing', 'Sale Fish', 'lightgreen', "**"..Player.PlayerData.name .. " (citizenid: "..Player.PlayerData.citizenid.." | id: "..Player.PlayerData.source..")** received $"..amount * illegalPrice[item.name].." for selling "..amount.."x "..QBCore.Shared.Items[item.name].label)

            -- Notification
            TriggerClientEvent('inventory:client:ItemBox', Player.PlayerData.source, QBCore.Shared.Items['bands'], 'add', amount * illegalPrice[item.name])
            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, 'You received '..amount * illegalPrice[item.name]..' Bands for selling your '..QBCore.Shared.Items[item.name].label, 'success', 2500)
        end
    end
end)
